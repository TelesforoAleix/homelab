...
| `23.0-endpoint-handover.md` | Phase 23.0 completion handover | **Written** 2026-09-13 — addressed to Phase 15.1 (next), 23.1, 23.2, 23.3, 19 and 14; outcome Complete. The endpoint live as `homelab-harness` on `127.0.0.1:8766` (eighth socket, 1.3), up after a locked reboot; ADR-048 accepted; **"the adapter interface is proved"** — the §8.1.3 check run locally and on the node, row 17's triple joined by request id; the `Result`-has-no-correlation-field finding to 23.3; the reservation-not-released debt to 15.1; two Workbench legacy-record defects onto the 18.2 finding; ten lessons incl. three paste failures |
| `23.0-endpoint.md` | Phase 23.0 brief | **Written** 2026-09-13, committed before implementation per ADR-017; **corrected during S1** (§6.6 priority enum and question truncation, §7.2 bot restart) **and S2** (§8 row 12: `/status` has no volume line); the fresh brief the Phase 23 header owed, for layers 1, 2 and 9. The always-running loopback endpoint as its own account; deterministic taxonomy; content-free audit line; the second adapter (Workbench→homelab) and the §8.1.3 check run for real; ADR-048 (model socket consumers as a group); Telegram client deferred with the reason |
...
| `15.1-gateway-and-spend-governor.md` | Phase 15.1 brief | **Written** 2026-09-13, committed before implementation per ADR-017. Vercel AI Gateway as `GatewayProvider` (plain HTTP, `LoadCredential=`) and the spend governor together (four windows, attended/unattended in money, reserve-at-max/settle-at-actual, fail closed, persisted); `utility` route only; approved-vendor list owner-signed; first real money, ≤ $1 for the phase under a $10/week gateway backstop. **Implemented and closed 2026-09-13**; §§6.5 and 8 row 8 amended at S1 (the per-request serving-provider pin) |
| `15.1-gateway-and-spend-governor-handover.md` | Phase 15.1 completion handover | **Written** 2026-09-13 — addressed to 23.1 (the `utility` route, measured cost and latency, unattended by definition), 15 (the registry, the ledger fields, the overshoot bound, the 400-mapping and price-drift debts), 23.3 (ledger schema; the governor is per-node) and 14 (`gateway-key` in the archive; `spend.json` loss resets windows); outcome Complete. Governor proved on the node, key revoked and replaced, **reconciled to nine decimals** (`$0.0000422`); four provider-reaching calls in the phase; the executor-cannot-fire-a-paying-call rule (ADR-049 §4) |
| `00.1-ram-upgrade.md` | Phase 00.1 brief | **Written** 2026-09-14, committed before implementation per ADR-017. A hardware sub-phase: 2 × 16 GB in place of 1 × 8 GB; prove on Linux before documenting; memtester as the pass gate; BIOS untouched; the firmware update out of scope |
| `00.1-ram-upgrade-handover.md` | Phase 00.1 completion handover | **Written** 2026-09-14 — addressed to 23.1 (next, unchanged), 14 (the inventory; 3-minute DHCP leases) and 13 (the pending firmware update); outcome Complete. 32 GB proved by `dmidecode`, `free` and a full `memtester` loop; the stress-test-is-an-outage lesson; the `poweroff` path observed by the watchdog; the spare 8 GB module |
...
| `13.1-agent-operator-access-handover.md` | Phase 13.1 handover | **Complete** 2026-09-13. `homelab-agent` exists, keyed, aliased, listed in `AllowUsers`; the grant is an explicit 172-entry list (sudo-rs allows no wildcard inside an argument); all sixteen rows OBSERVED, row 12 by the executor over `ssh homelab-agent`. Addressed to 15.1 (alias, staging names, `cp -p`, the S3 rewrite), every later phase (the AGENT/OWNER convention) and 14 (two paths, one exclusion) |
| `13.1-s2-stage-report.md` | Phase 13.1 S2 stage report | **Landed** 2026-09-13. The gate fired on the S1 grant (sudo-rs: no wildcards in arguments); the list landed on the second attempt; 27 allows, 24 denies, 4 exactness probes; sshd reloaded with a root shell held; backup PASS; row 12 by the executor. sudo-rs facts, verifier quirks, two MacBook-side slips |
| `13.1-s1-stage-report.md` | Phase 13.1 S1 stage report | **Landed** 2026-09-13. Files and local proof, no node change: the sudoers grant (parsed OK on macOS C sudo; sudo-rs on the node is the gate), the installer, the alias, backup coverage, both standards, the guide skeleton and the S2 runbook in AGENT/OWNER form. Three brief amendments (644 not 600; `cp -p`; glob-hole denies) and the reasons |
| `13.1-agent-operator-access.md` | Phase 13.1 brief | **Written** 2026-09-13, committed before implementation per ADR-017. A `homelab-agent` account with its own key and a closed `NOPASSWD` list (ADR-049) so executors run and read routine node steps themselves; the owner keeps passwords, secrets, browser, phone and lockout-class steps. Two stages; sixteen rows; the sshd `AllowUsers` edit is the one lockout-class step |
