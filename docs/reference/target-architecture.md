...
answers **what is this system supposed to be when it is finished**, and without that the phases drift
into each other — which is what happened: Phase 23 accumulated seven objectives that were really four
phases, two of which belonged to Phase 15.

The format is borrowed deliberately. Each layer below states its responsibility, what it must **not**
do, what it receives and hands on, what exists today, what constrains it, how it behaves when
something it depends on is missing, and what is still open. That structure comes from the Odysseus
project's `specs/` directory, which keeps *"compact maps of current implementation truth"* separate
from historical decision records. We had ADRs and a roadmap and nothing in between.
...
while the system is busy queues; it does not double-execute — an in-process scheduler that
double-fires under multiple processes is a known failure in comparable systems.
...
that they would not have required otherwise. This idea is borrowed from Odysseus and it is better than
a static label, because it makes the marking do work rather than decorate a prompt.
...
fallback, proved against a genuinely exhausted provider. Phase 15 *"has already accidentally
started"*. What does not exist is models as **configuration** rather than two entries in Python.
...
message — proved already in Phase 09.
...
- **One process holding everything.** The comparable system we studied is a modular monolith whose own
  threat model concedes that shell and filesystem tools run unsandboxed as the application user, and
  that prompt-level defences are not a process boundary. We have spent phases building the opposite;
  adopting that shape would undo ADR-011 and ADR-025 without anyone noticing until something reaches
  the host.
- **A database, yet.** Plural persistence — relational plus JSON plus files plus a vector store —
  produced multiple sources of truth, backup gaps and divergence in the system we studied. Files stay
  canonical until they demonstrably fail.
- **A central registry of anything for other adopters.** Factory is fork-and-customise software.

## 7. Named failures to design against

Taken from a system that has actually run, and worth more than its features:

1. **Registry duplication.** Tool descriptions, schemas, aliases, handlers, dispatch entries, UI
   controls and retrieval metadata maintained separately, so a tool can exist in one and be missing
   from another. The same class of bug as the two YAML parsers we removed from the dashboard.
2. **Tool discovery treated as authorization.** Selecting a relevant subset of tools is a *context
   economy* problem; permitting execution is a *security* problem. They must be separate mechanisms,
   and only the second is a boundary.
3. **In-process schedulers double-firing** under multiple processes.
4. **Owner scoping retrofitted** onto a single-user system, producing null-owner edge cases forever.
   We are single-user; that should be an explicit decision rather than an accident.
5. **Compatibility façades** left in place during refactoring, creating several apparent homes for one
   responsibility.
...
- **Layer 7** — Phase 15, Phase 15.0 and Phase 23 all claim model registry and routing. Phase 23's
  brief hedges (*"build it here if 15.0 has not run"*), which is muddle rather than a decision.
...
would replace. ADR-034 and the Workbench answered most of that question by building it.
