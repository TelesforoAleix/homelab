...
| Factory Workbench (server) | **Active** | Factory `2e82f51`; `factory-ops` `66283c2` | Phase 18.2 deliverable (ADR-036, ADR-038) | `homelab-workbench.service`, `User=aleix`, `NoNewPrivileges`, `1.3 OK`. `python3 -m workbench.cli --project /srv/homelab/projects/factory serve` with `PYTHONPATH=/srv/homelab/factory` (no `WorkingDirectory=` — see the standard). Binds `127.0.0.1:8765` only; reached via `ssh homelab-workbench`. Tied to the volume: skipped while locked, started by unlock. Writes `ops/` records and never commits. **Phase 23.0:** Factory `9986188` (merged to `main` at close) adds the `homelab` adapter, the `adapter` key and `cli run`; the node's clone follows `main` |
| Harness — the endpoint (server) | **Active** | `services/homelab-harness/` at the 23.0 close; Python 3.14.4 stdlib only | Phase 23.0 deliverable (ADR-038 §2, ADR-034 §11, ADR-048) | `homelab-harness.service`, `User=homelab-harness`, `1.3 OK`, `127.0.0.1:8766`, `StateDirectory` on root. Reaches the model helper's socket as a member of `homelab-model`. No dependency beyond the standard library |
| Vercel AI Gateway (external, metered) | **Active** since Phase 15.1 (2026-09-13) | Chat Completions API at `https://ai-gateway.vercel.sh/v1/chat/completions`, as documented 2026-09-13 (no version string — the docs date is the version); models `openai/gpt-5.6-luna` (routed), `openai/gpt-5.6-sol` (unrouted) | ADR-033 | Reached by the helper with `urllib`/`ssl` only; key under `LoadCredential=`; per-request serving-provider pin `providerOptions.gateway.only=["openai"]`; governed by `spend.py`. $10/week key budget is the backstop |
