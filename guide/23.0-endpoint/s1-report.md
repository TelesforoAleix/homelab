...
| 6.3 | state on root = `audit.jsonl` only; results are the client's | **Accepted.** | `Audit.FIELDS` is the whole of what is persisted; content cannot be added without failing the fixture. No `RequiresMountsFor=` — the installer asserts it empty after `daemon-reload`. |
...
- `systemctl show -p RequiresMountsFor --value homelab-harness.service` → empty.
